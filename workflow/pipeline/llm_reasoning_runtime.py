...
from src.utils import get_completion_vllm, multi_thread_task_dict
...
class Utils:
    ...
    def get_config(retrieved_data):
        ...
        for size in ["llm_8b", "llm_3b", "llm_1b"]:
        ...
        for size in ["beams_1", "beams_3", "beams_5", "beams_10"]:
...
def parse_args():
    ...
    parser.add_argument("--num-workers", type=int, required=True, default=1)
...
if __name__ == "__main__":
    ...
    data = {k: samples[: args.num_samples] for k, samples in data.items()}
    for partition, samples in data.items():
        ...
        for sample in samples:
            sample["llm_reasoning"] = results[sample["id"]]["llm_call"]
            sample["runtime"] = {
                "start_time": results[sample["id"]]["start_time"],
                "end_time": results[sample["id"]]["end_time"],
            }
